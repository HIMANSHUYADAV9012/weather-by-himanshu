...
function playWeatherMusic(condition) {
  ...
  if (currentTrack !== musicFile) {
    ...
    // 👇 ADD THIS LINE to show popup immediately
...
function toggleMusic() {
  ...
  const updateControls = () => {
    document.getElementById("musicControl").innerHTML = isMusicPlaying
      ? '<i class="fas fa-pause"></i>'
      : '<i class="fas fa-play"></i>';
  };
    ...
    if (audioContext.state === "suspended") {
        ...
        .then(() => {
          audioElement
            ...
            .then(() => {
              isMusicPlaying = true;
              updateControls();
              document.getElementById("musicPlayer").style.display = "flex"; // 👈 Also add here
            })
            .catch((playError) => {
      ...
      audioElement
        ...
        .then(() => {
          isMusicPlaying = true;
          updateControls();
          document.getElementById("musicPlayer").style.display = "flex"; // 👈 And here
        })
        .catch((playError) => {
