...
async function getWeather(location = null) {
    ...
    try {
        // Single optimized API call with error handling
...
// Display weather data with optimized DOM updates
function displayWeatherData(data, location) {
    ...
    // Generate HTML
    const weatherHTML = generateWeatherHTML(data, condition, emoji, isDay, sunrise, sunset);
    
    // Update DOM efficiently
    const weatherContainer = document.createElement('div');
    weatherContainer.className = 'weather-card bg-white/30 backdrop-blur-md rounded-2xl p-6 shadow-xl w-full max-w-md animate-fadeIn text-gray-900 relative';
...
// Generate weather HTML template
function generateWeatherHTML(data, condition, emoji, isDay, sunrise, sunset) {
    ...
    return `
        <div class="relative z-10">
            ...
            <div class="hourly-forecast-container mt-6 w-full">
...
// Optimized hourly forecast generation
...
function getTheme(condition, isDay) {
    if (!isDay && (condition.includes("clear") || condition.includes("sun") || 
        condition.includes("cloud") || condition.includes("thunder"))) {
        return "theme-night";
    }
    return getThemeFromCondition(condition);
}

// Optimized search history update
